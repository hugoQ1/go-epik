...
func init() {
	if s := os.Getenv("EPIK_CHAIN_TIPSET_CACHE"); s != "" {
		tscs, err := strconv.Atoi(s)
		if err != nil {
			log.Errorf("failed to parse 'EPIK_CHAIN_TIPSET_CACHE' env var: %s", err)
		}
		DefaultTipSetCacheSize = tscs
	}

	if s := os.Getenv("EPIK_CHAIN_MSGMETA_CACHE"); s != "" {
		mmcs, err := strconv.Atoi(s)
		if err != nil {
			log.Errorf("failed to parse 'EPIK_CHAIN_MSGMETA_CACHE' env var: %s", err)
		}
...
func (cs *ChainStore) readAMTCids(root cid.Cid) ([]cid.Cid, error) {
	...
	a, err := blockadt.AsArray(cs.ActorStore(ctx), root, adt.DefaultMsgAmtBitwidth)
	if err != nil {
...
func (cs *ChainStore) GetParentReceipt(b *types.BlockHeader, i int) (*types.MessageReceipt, error) {
	ctx := context.TODO()
	// block headers use adt0, for now.
	a, err := blockadt.AsArray(cs.ActorStore(ctx), b.ParentMessageReceipts, adt.DefaultMsgAmtBitwidth)
	if err != nil {
		return nil, xerrors.Errorf("amt load: %w", err)
	}
