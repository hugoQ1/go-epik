...
func init() {
	coalesceTipsets = os.Getenv("EPIK_SYNC_FORMTS_PEND") == "yes"

	if bootstrapPeerThreshold := os.Getenv("EPIK_SYNC_BOOTSTRAP_PEERS"); bootstrapPeerThreshold != "" {
		threshold, err := strconv.Atoi(bootstrapPeerThreshold)
		if err != nil {
			log.Errorf("failed to parse 'EPIK_SYNC_BOOTSTRAP_PEERS' env var: %s", err)
		} else {
