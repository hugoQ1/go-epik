...
func (mp *MessagePool) checkBalance(m *types.SignedMessage, curTs *types.TipSet) error {
	...
	if balance.LessThan(requiredFunds) {
		return xerrors.Errorf("not enough funds (required: %s, balance: %s): %w", types.EPK(requiredFunds), types.EPK(balance), ErrNotEnoughFunds)
	}
	...
	if balance.LessThan(requiredFunds) {
		...
		// be out of sync.
		return xerrors.Errorf("not enough funds including pending messages (required: %s, balance: %s): %w", types.EPK(requiredFunds), types.EPK(balance), ErrSoftValidationFailure)
	}
