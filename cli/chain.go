...
var chainCmd = &cli.Command{
	Name:  "chain",
	Usage: "Interact with epik blockchain",
	...
	Action: func(cctx *cli.Context) error {
		...
		if cctx.Bool("gas-stats") {
			...
			for i, ts := range tss {
				...
				fmt.Printf("%d: %d blocks (baseFee: %s -> maxFee: %s)\n", ts.Height(), len(ts.Blocks()), ts.Blocks()[0].ParentBaseFee, types.EPK(types.BigMul(pbf, types.NewInt(uint64(build.BlockGasLimit)))))
