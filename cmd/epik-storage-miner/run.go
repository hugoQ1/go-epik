...
var runCmd = &cli.Command{
	Name:  "run",
	Usage: "Start a epik miner process",
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "miner-api",
			Usage: "2345",
	...
	Action: func(cctx *cli.Context) error {
		...
		if !ok {
			return xerrors.Errorf("repo at '%s' is not initialized, run 'epik-miner init' to set it up", minerRepoPath)
		}
		...
		rpcServer.Register("EpiK", apistruct.PermissionedStorMinerAPI(metrics.MetricedStorMinerAPI(minerapi)))
		...
		srv := &http.Server{
			...
			BaseContext: func(listener net.Listener) context.Context {
				ctx, _ := tag.New(context.Background(), tag.Upsert(metrics.APIInterface, "epik-miner"))
				return ctx
			},
		}
