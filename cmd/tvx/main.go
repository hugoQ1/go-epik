...
func main() {
   ...
   tvx exec executes test vectors against Epik. Either you can supply one in a
   ...
   2. If you're running tvx against a local Epik client, you can set the REPO
      env variable to have the API endpoint and token extracted from the repo.
      Alternatively, you can pass the --repo CLI flag.

   3. Rely on the default fallback, which inspects ~/.epik and extracts the
      API endpoint string if the location is a Epik repo.
...
func initialize(c *cli.Context) error {
	// EPIK_DISABLE_VM_BUF disables what's called "VM state tree buffering",
	// which stashes write operations in a BufferedBlockstore
	// (https://github.com/EpiK-Protocol/go-epik/blob/b7a4dbb07fd8332b4492313a617e3458f8003b2a/lib/bufbstore/buf_bstore.go#L21)
	...
	if FullAPI, Closer, err = lcli.GetFullNodeAPI(c); err != nil {
		err = fmt.Errorf("failed to locate EpiK node; err: %w", err)
	}
	return err
}
