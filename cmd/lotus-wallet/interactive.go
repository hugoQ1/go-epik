...
func (c *InteractiveWallet) WalletSign(ctx context.Context, k address.Address, msg []byte, meta api.MsgMeta) (*crypto.Signature, error) {
	err := c.accept(func() error {
		...
		case api.MTChainMsg:
			...
			fmt.Println("Value:", types.EPK(cmsg.Value))
			fmt.Println("Max Fees:", types.EPK(cmsg.RequiredFunds()))
			fmt.Println("Max Total Cost:", types.EPK(big.Add(cmsg.RequiredFunds(), cmsg.Value)))
			...
			if c.apiGetter != nil {
				...
				if builtin.IsMultisigActor(toact.Code) && cmsg.Method == multisig.Methods.Propose {
					...
					fmt.Println("\tMultiSig Proposal Value:", types.EPK(mp.Value))
