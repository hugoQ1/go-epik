...
var consensusCheckCmd = &cli.Command{
   ...
   epik-shed consensus check --height @common --lookback 10

   Calculate the expected tipset height and look back 10 tipsets
   epik-shed consensus check --height @expected --lookback 10

   Check if nodes all share a common genesis
   epik-shed consensus check --height 0

   Check that all nodes agree upon the tipset for 1day post genesis
   epik-shed consensus check --height 2880 --lookback 0
	`,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "height",
