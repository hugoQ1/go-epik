...
var keyinfoImportCmd = &cli.Command{
	Name:  "import",
	Usage: "import a keyinfo file into a epik repository",
	Description: `The import command provides a way to import keyfiles into a epik repository
   without running the daemon.

   Note: The EPIK_PATH directory must be created. This command will not create this directory for you.

   Examples

   env EPIK_PATH=/var/lib/epik epik-shed keyinfo import libp2p-host.keyinfo`,
	Action: func(cctx *cli.Context) error {
...
var keyinfoNewCmd = &cli.Command{
   ...
   Both the bls and secp256k1 keyfiles can be imported into a running epik daemon using
   the 'epik wallet import' command. Or imported to a non-running / unitialized repo using
   the 'epik-shed keyinfo import' command. Libp2p host keys can only be imported using epik-shed
   as epik itself does not provide this functionality at the moment.`,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "output",
