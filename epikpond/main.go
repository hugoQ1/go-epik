...
var onCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		var cmd *exec.Cmd
		if !node.Storage {
			cmd = exec.Command("./epik", cctx.Args().Slice()[1:]...)
			cmd.Env = []string{
				"EPIK_PATH=" + node.Repo,
			}
		} else {
...
var runCmd = &cli.Command{
	Name:  "run",
	Usage: "run epikpond daemon",
	Action: func(cctx *cli.Context) error {
		rpcServer := jsonrpc.NewServer()
		a := &api{running: map[int32]*runningNode{}}
		rpcServer.Register("Pond", a)

		http.Handle("/", http.FileServer(http.Dir("epikpond/front/build")))
		http.HandleFunc("/app/", func(w http.ResponseWriter, r *http.Request) {
			http.ServeFile(w, r, "epikpond/front/build/index.html")
		})
