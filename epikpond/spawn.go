...
func (api *api) Spawn() (nodeInfo, error) {
	dir, err := ioutil.TempDir(os.TempDir(), "epik-")
	if err != nil {
		return nodeInfo{}, err
	}
	...
	if id == 1 {
		...
		// make genesis
		genf, err := ioutil.TempFile(os.TempDir(), "epik-genesis-")
		if err != nil {
			return nodeInfo{}, err
		}

		api.genesis = genf.Name()
		genParam = "--epik-make-genesis=" + api.genesis
...
func (api *api) SpawnStorage(fullNodeRepo string) (nodeInfo, error) {
	dir, err := ioutil.TempDir(os.TempDir(), "epik-storage-")
	if err != nil {
		return nodeInfo{}, err
	}
